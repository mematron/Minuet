...
## System Architecture & Cognitive Lifecycle

Arminta operates not as a linear loop, but as a multi-layered cognitive topology. Telemetry is parsed probabilistically, evaluated for causal relationships over distinct time horizons, and optimized via an isolated asynchronous meta-cognition layer.
...
1. **BayesianPerception**: Instead of reacting immediately to volatile spikes in CPU or RAM, telemetry is passed through sequential probabilistic updates. This dampens transient noise, ensuring that mitigation routines are only executed when a true state change has occurred.
2. **WorkingMemory**: Acts as the volatile immediate scratchpad. It maintains a short-term rolling buffer of system events, telemetry deltas, and recent interventions, serving as the primary dataset for short-horizon evaluation.
3. **EpisodicMemory**: Built on a structured SQLite backend (`arminta_episodic.db`). It logs long-term operational history, tracking specific context footprints, executed actions, anomalies, and whether a chosen vector stabilized the host environment or caused a fault.
4. **EmotionalState**: Floats an internal systemic affect vector that decays over time. High-stress triggers (e.g., severe IO wait, critical thermal thresholds, or unmitigated PSI stalls) skew these parameters, lowering the threshold for aggressive action and narrowing down safety margins to prioritize system survival.
5. **WorldModel**: Maps structural state-action association statistics over deep operational life spans. It goes beyond simple tabular reinforcement learning, allowing the agent to contextualize actions based on overall environmental topology rather than flat thresholds.
6. **HypothesisEngine**: Executes a continuous Genetic Algorithm (GA) over the nodes of the causal graph. It postulates potential new edge relationships among disparate variables (e.g., correlating specific hardware IRQ storms with unexpected browser background tab behavior).
7. **DreamCycle**: An asynchronous maintenance process triggered exclusively during resource idle periods. The DreamCycle handles resource-intensive housekeeping: pruning dead or weak causal links, validating pending hypotheses against historical data, and driving the MetaCognition engine.

---

### Causal Execution & Paramorphic Learning Loops

Arminta v2 relies on dual-horizon feedback structures to verify its own impacts and rewrite its configuration without stopping execution:
